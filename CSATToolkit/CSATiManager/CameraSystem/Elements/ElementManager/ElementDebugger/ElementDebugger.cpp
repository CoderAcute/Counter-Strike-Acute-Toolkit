...
bool ElementDebugger::FreeCameraPath_Draw(const std::shared_ptr<FreeCameraPath> const Path) {
    if (!Path) {
        this->CSATi->IDebugger().AddError("目标路径指针为空，无法绘制！");
        return false;
    }
    if (!Path->Drawable) {
        return false;
    ...
    const std::vector<CSATMath::CameraKeyFrame>& Frames = Path->GetAllKeyFrames();
    ...
    for (int i = 0; i < Size; ++i) {
        if (!this->AL3D->GetViewMatrix())return false;
        const auto& Frame = Frames.at(i);
        // 绘制关键帧的摄像机
        std::string label = Path->Name + " #" + std::to_string(i);
        this->CamDrawer->DrawCamera(Frame.SpatialState.GetPosition(), Frame.SpatialState.GetRotationEuler(), label.c_str());
        ...
        if (!firstFrame) {
            ...
            CSATMath::XMWorldToScreen(
                prevPosition, prevScreenPos,
                this->AL3D->GetViewMatrix(),
                this->AL3D->GetWinWidth(),
                this->AL3D->GetWinHeight()
            );

            CSATMath::XMWorldToScreen(
                Frame.SpatialState.GetPosition(), currentScreenPos,
                this->AL3D->GetViewMatrix(),
                this->AL3D->GetWinWidth(),
                this->AL3D->GetWinHeight()
            );
